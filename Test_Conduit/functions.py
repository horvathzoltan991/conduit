...
def logout(browser):
    log_out_btn = WebDriverWait(browser, 15).until(EC.presence_of_element_located((By.XPATH, '//ul/li[5]/a')))
    log_out_btn.click()


def publish_article(browser, title, summary, text, tag):
    new_article_btn = WebDriverWait(browser, 15).until(
        EC.presence_of_element_located((By.XPATH, '//a[@href="#/editor"]')))
    new_article_btn.click()
    article_title = WebDriverWait(browser, 15).until(
...
def add_comment(browser, comment):
    ...
    first_post.click()
    comment_textarea = WebDriverWait(browser, 15).until(
...
def navigate_to_settings(browser):
    settings_btn = WebDriverWait(browser, 15).until(
        EC.presence_of_element_located((By.XPATH, '//a[@href="#/settings"]')))
    settings_btn.click()


def change_profile_pic(browser, url):
    picture_link_input = WebDriverWait(browser, 15).until(
...
def delete_comment(browser):
    delete_btn = WebDriverWait(browser, 15).until(
